...
constexpr size_t MAP_INIITAL_CAPACITY = FF_cob__map_initial_capacity;
...
template <typename Key_T, typename Value_T, typename Hash_T = std::hash<Key_T>> class FlatHashMap {
public:
    struct Slot {
        Key_T key = {};
        Value_T value = {};
    ...
    explicit FlatHashMap(size_t capacity = MAP_INIITAL_CAPACITY);
    ~FlatHashMap() = default;
    ...
    template <typename... Args_T> Value_T *emplace(const Key_T &key, Args_T &&...args) {
