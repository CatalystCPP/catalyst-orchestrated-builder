...
\lhead{\small\color{slate}Catalyst Build Executor}
...
{\Huge\bfseries\color{accentdark} Catalyst Build Executor}\\[8pt]
...
\begin{flushleft}
\large
\textbf{Subject:} \texttt{cbe} v0.5.0 --- The Catalyst Build Executor\\[4pt]
...
CBE (the \emph{Catalyst Build Executor}) is a specialized, low-overhead execution backend for machine-generated build manifests emitted by the Catalyst meta-build system. Operating in the same structural layer as Ninja beneath CMake or Meson, CBE's architectural boundary separates high-level meta-configuration (such as profile resolution, compiler toolchain mapping, and dependency package retrieval) from the mechanical execution of directed acyclic dependency graphs (DAGs).

This decoupling is the foundational design constraint of the system. The meta-build layer processes compiler profiles and emits a flat, pipe-delimited manifest. CBE is optimized for a single objective: ingest this manifest and drive the implied transformations with minimal scheduling overhead, while enforcing robust incremental rebuilding guarantees.
...
{CBE operates strictly as a low-abstraction mechanical execution layer.
By delegating compiler profile resolution and custom macro expansion to the Catalyst meta-build generator, the executor operates over regular, pipe-delimited targets, bypassing the computational overhead of generic macro parsing and variable evaluation loops.}

% =====================================================================
\section{System Architecture}

The execution path is a short, linear pipeline. The CLI parses arguments into an
\lstinline|ExecutorConfig|; the parser populates a \lstinline|CBEBuilder|
(either from the binary cache or the text manifest); the builder hands a
finished \lstinline|BuildGraph| to the \lstinline|Executor|, which either runs
the parallel build or emits one of several auxiliary artifacts.
...
\begin{tikzpicture}[
  ...
  box/.style={draw=accent,fill=boxbg,rounded corners,line width=0.7pt,
    align=center,inner sep=6pt,minimum height=2.0em,text width=2.6cm},
  io/.style={draw=slate,fill=codebg,rounded corners,line width=0.6pt,
    align=center,inner sep=5pt,text width=2.3cm,font=\footnotesize},
  ar/.style={-{Stealth[length=2.2mm]},line width=0.7pt,accentdark}
]
\node[io] (manifest) {\texttt{catalyst.build}\\\footnotesize(text manifest)};
\node[io,below=4mm of manifest] (bin) {\texttt{.catalyst.bin}\\\footnotesize(binary cache)};

\node[box,right=8mm of manifest,yshift=-3.5mm] (parse) {Parser\\\footnotesize text / binary};
\node[box,right=7mm of parse] (builder) {CBEBuilder\\\footnotesize facade};
\node[box,right=7mm of builder] (graph) {BuildGraph\\\footnotesize nodes + edges};
\node[box,right=7mm of graph] (exec) {Executor\\\footnotesize scheduler};

\node[io,right=8mm of exec,yshift=9mm] (out1) {build outputs};
\node[io, right=8mm of exec, text width=3.3cm] (out2) {compile\_commands};
\node[io,right=8mm of exec,yshift=-9mm] (out3) {DOT graph};

\draw[ar] (manifest) -- (parse);
\draw[ar] (bin) -- (parse);
\draw[ar] (parse) -- (builder);
\draw[ar] (builder) -- (graph);
\draw[ar] (graph) -- (exec);
\draw[ar] (exec) -- (out1);
\draw[ar] (exec) -- (out2);
\draw[ar] (exec) -- (out3);
...
\node[draw=codeframe,rounded corners,fill=codebg,fit=(parse)(exec),
  inner sep=10pt,label={[slate,font=\footnotesize]above:CBE core}] {};
\end{scope}
\end{tikzpicture}}
\end{center}
...
\texttt{cbe/utility.hpp} establishes the error-handling vocabulary for the whole
...
The distinction between the three input kinds is the heart of CBE's
dependency model:
...
\lstinline|Definitions| map (with a future transition to a cache-friendly sorted flat map planned), again keyed and valued by views:
...
This is handled by a type-erased resource list. \lstinline|CBEBuilder| and
...
The manifest is line-oriented and pipe-delimited, chosen for trivially fast
parsing rather than human authorship:

\begin{itemize}
  \item Lines beginning with \lstinline|#| are comments.
  \item \lstinline|DEF|\,\lstinline=|=\,\textit{key}\,\lstinline=|=\,\textit{value}
  declares a definition.
  \item Any other line is a step: \textit{tool}\,\lstinline=|=\,\textit{inputs}\,%
  \lstinline=|=\,\textit{output}.
\end{itemize}
...
malformed lines yield an error rather than recovery.
...
with a \emph{string table} appended at the end of the file. All string-bearing
...
identical views collapse to a single entry via a
\lstinline|unordered_map<string_view, StringRef>| cache --- so the table is
...
platform-tagged (\texttt{CATBL001} on Linux, \texttt{CATBM001} on macOS, and \texttt{CATBW001} on Windows) so
...
\lstinline|needs_rebuild| is the freshness check. A step rebuilds if its output
is missing, or if the manifest, any parsed input, any depfile header, or any
opaque dependency is at least as new as the output. Timestamp lookups are
funneled through the \lstinline|StatCache| (\S\ref{sec:statcache}) to avoid
repeated \lstinline|stat| calls on shared headers. Notably the manifest itself
is a dependency of every step --- changing build flags correctly invalidates
everything.
...
Behind the \lstinline|heterogenous_core_affinity| feature flag, the worker biases
...
\lstinline|lower_bound| binary search remains valid). It is the one place CBE
...
\begin{itemize}
  ...
  topological order. CBE ships its own minimal JSON value type
...
CBE is aggressively feature-gated through \lstinline|FF_cbe__*| preprocessor
...
To quantitatively evaluate the architectural efficiency of CBE, comparative benchmarks were executed against GNU Make (v4.3) and Ninja (v1.11.1) on a high-density mock repository (\texttt{heavy\_repo}) designed to simulate a massive C++ codebase. The workload consists of 1,000 translation units arranged in a balanced binary tree dependency topology to maximize scheduler breadth and depth.

All benchmarks were captured using \texttt{hyperfine} with 2 warmup runs. The tests were performed on an AMD Ryzen 9 5950X (16 physical cores, 32 hardware threads, P-core/E-core affinity flags disabled for standardization) running Linux Kernel 6.5.

\subsection{Full Incremental Build Execution}

The primary benchmark measures the makespan of a full incremental build loop, wherein a clean workspace is built from scratch. For this comparison, GNU Make and Ninja were executed with maximum parallel job parameters matching the hardware capabilities (\texttt{-j32}), while CBE utilized its default automated core-detection scheduler.
...
\midrule
\texttt{cbe}   & $1.625 \pm 0.021$ & $1.593$ & $1.652$ & $1.00$ \\
\texttt{ninja} & $1.801 \pm 0.010$ & $1.787$ & $1.817$ & $1.11 \pm 0.02$ \\
\texttt{make}  & $1.846 \pm 0.016$ & $1.828$ & $1.875$ & $1.14 \pm 0.02$ \\
\bottomrule
\end{tabular}
\end{center}

CBE achieves a statistically significant 11\% speedup over Ninja and a 14\% speedup over GNU Make. This acceleration is driven by three architectural vectors:
\begin{enumerate}
    \item \textbf{Binary Serialization Warm Path}: CBE bypasses text-based parsing of the build manifest on subsequent runs, loading the pre-compiled \texttt{.catalyst.bin} via in-place \lstinline|reinterpret_cast| pointers on memory-mapped pages.
...
To ensure complete transparency and prevent overstating CBE's performance profile, secondary tasks were benchmarked to identify overhead profiles. These tasks do not run compiler processes and are governed entirely by metadata read/write performance.
...
\quad\texttt{ninja -t clean} & $12.3 \pm 2.4$ & $9.7$ & $15.7$ & $1.00$ \\
\quad\texttt{cbe -t clean}   & $23.2 \pm 2.9$ & $20.3$ & $30.1$ & $1.89 \pm 0.44$ \\
\quad\texttt{make clean}      & $367.8 \pm 10.2$ & $353.4$ & $381.8$ & $29.94 \pm 5.88$ \\
\midrule
\textbf{CompDB Generation} & & & & \\
\quad\texttt{ninja -t compdb} & $4.4 \pm 0.7$ & $2.8$ & $5.8$ & $1.00$ \\
\quad\texttt{cbe -t compdb}   & $12.9 \pm 1.8$ & $9.6$ & $17.0$ & $2.92 \pm 0.63$ \\
\bottomrule
\end{tabular}
\end{center}

On clean-only and JSON compilation database (CompDB) emission, Ninja outperforms CBE. This performance gap is explained by Ninja's internal metadata organization:
\begin{itemize}
    \item \textbf{Internal Database Caching}: Ninja maintains a highly optimized, compact, append-only binary log (\texttt{.ninja\_log} and \texttt{.ninja\_deps}) directly inside its core execution loop, bypassing high-level standard library abstractions like \lstinline|std::filesystem| or \lstinline|std::flat_map| used by CBE.
    \item \textbf{Format Simplicity}: The Catalyst manifest requires parsing comma-separated lists and resolving multiple definition sets, whereas Ninja reads a pre-calculated, flat dependency structure designed for rapid traversal.
\end{itemize}
CBE, however, remains nearly an order of magnitude faster than GNU Make during workspace cleanup, because GMake must recursively traverse directories and evaluate rules sequentially.
