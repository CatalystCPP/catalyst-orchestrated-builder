...
\begin{flushleft}
\large
\textbf{Subject:} \texttt{cob} v0.3.0 --- The Catalyst Orchestrated Builder \\[4pt]
...
\midrule
\texttt{cob}   & $1.625 \pm 0.021$ & $1.593$ & $1.652$ & $1.00$ \\
...
COB achieves a statistically significant 11\% speedup over Ninja and a 14\% speedup over GNU Make.
...
\quad\texttt{cob -t clean}   & $23.2 \pm 2.9$ & $20.3$ & $30.1$ & $1.89 \pm 0.44$ \\
...
\quad\texttt{cob -t compdb}   & $12.9 \pm 1.8$ & $9.6$ & $17.0$ & $2.92 \pm 0.63$ \\
\bottomrule
\end{tabular}
\end{center}

On clean-only and JSON compilation database (CompDB) emission, Ninja outperforms COB. This performance gap is explained
by Ninja's internal metadata organization:
\begin{itemize}
    \item \textbf{Internal Database Caching}: Ninja maintains a highly optimized, compact, append-only binary log (\texttt{.ninja\_log} and \texttt{.ninja\_deps}) directly inside its core execution loop, bypassing high-level standard library abstractions like \lstinline|std::filesystem| or \lstinline|std::flat_map| used by COB.
